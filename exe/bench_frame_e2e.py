...
_ras = raster(N)
# 어텐션은 앵커 512 개짜리라 연산이 아니라 **커널 실행 횟수**가 비용이다. 모양이
# 매 프레임 같으므로 CUDA 그래프로 한 번 잡아두고 재생하면 그 비용이 사라진다.
# 값은 그대로다. 잡기는 난수를 쓰는 것들(raster 의 색)을 다 만든 **뒤에** 해야
# 한다 -- 잡는 도중 난수 오프셋이 움직이면 실패한다.
