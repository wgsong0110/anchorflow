...
[2026-09-20] **GF 와 맞추는 용도로는 이 파일을 쓰지 말 것.** 여기 재질 4(cdmpm)
는 GF 의 NACC 를 옮긴 것이지만 항복면 위의 경화 블록이 빠져 있고, 전달 방식(APIC)
과 격자 힘 항(MLS)도 GF 와 다르다. 같은 궤적을 내야 하면 `exe/gf_mpm.py` 를 쓴다.
이 파일은 합성 씬을 빨리 만들어 보는 용도로만 남긴다.
"""
