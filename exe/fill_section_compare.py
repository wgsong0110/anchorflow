...
ap.add_argument("--mesh", type=int, default=1,
                help="1이면 marching cubes 메시 내부를 채운다")
...
# 3DGS -> 메시 -> 메시 내부를 포아송 디스크 입자셋으로 채운다
from anchorflow.psfill import poisson_fill, poisson_fill_mesh      # noqa: E402

if a.mesh:
    L = poisson_fill_mesh(S, spacing=a.spacing, grid=a.grid, sigma=a.sigma,
                          level=a.level, mesh_out=a.out.replace(".mp4", ".obj"))
else:
    L = poisson_fill(S, spacing=a.spacing, grid=a.grid, level=a.level)
tree = cKDTree(S)
...
thick = a.thick if a.thick > 0 else a.spacing
C0 = 0.28209479177387814

P_s = trans.detach().cpu().numpy()
P_l = L
col_s = (0.5 + C0 * shs[:, 0, :].detach().cpu().numpy()).clip(0, 1)
col_l = (0.5 + C0 * shs_f[:, 0, :].detach().cpu().numpy()).clip(0, 1)

z_lo = float(min(P_s[:, AX].min(), P_l[:, AX].min()))
z_hi = float(max(P_s[:, AX].max(), P_l[:, AX].max()))
# 전 프레임 공통 화면 범위 (스윕 중에 배율이 흔들리지 않게)
u_lo = min(P_s[:, U].min(), P_l[:, U].min()) - 0.03
u_hi = max(P_s[:, U].max(), P_l[:, U].max()) + 0.03
v_lo = min(P_s[:, V].min(), P_l[:, V].min()) - 0.03
v_hi = max(P_s[:, V].max(), P_l[:, V].max()) + 0.03
span = max(u_hi - u_lo, v_hi - v_lo)
uc, vc = 0.5 * (u_lo + u_hi), 0.5 * (v_lo + v_hi)
u_lo, u_hi = uc - span / 2, uc + span / 2
v_lo, v_hi = vc - span / 2, vc + span / 2
R = a.res
print(f"[단면] 축 {'xyz'[AX]}  범위 {z_lo:.3f} ~ {z_hi:.3f}  두께 {thick:.4f}  "
      f"화면 {span:.3f}", flush=True)
...
for i in tqdm(range(a.frames), desc="단면", ncols=80):
    ...
    ma = np.abs(P_s[:, AX] - z0) < thick * 0.5
    mb = np.abs(P_l[:, AX] - z0) < thick * 0.5
    a_img = scatter([(P_s[ma], col_s[ma])])
    c_img = scatter([(P_l[mb], col_l[mb])])
    b_img = scatter([(P_s[ma], col_s[ma]), (P_l[mb], col_l[mb])])
    row = []
    for nm, im in ((f"원본 3DGS 단면   z={z0:.3f}   커널 {int(ma.sum())}", a_img),
                   (f"채운 입자셋만   z={z0:.3f}   {int(mb.sum())}개", c_img),
                   (f"합친 단면   z={z0:.3f}   {int(ma.sum()) + int(mb.sum())}개", b_img)):
