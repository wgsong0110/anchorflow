...
class Pipe:
    ...
    compute_cov3D_python = True       # <- standard scales+rotations path
    debug = False
...
def main():
    ...
    _fea_dim = len([p for p in _PD.read(args.ply)["vertex"].properties if p.name.startswith("fea")])
    g = GaussianModel(sh, fea_dim=_fea_dim); g.load_ply(args.ply); g.active_sh_degree = sh
