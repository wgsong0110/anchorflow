...
mkdir -p $W/bench_ipg$DTM
lo=${SEEDS%%-*}; hi=${SEEDS##*-}
for sd in $(seq $lo $hi); do
  S=$(printf "%02d" $sd)
  OUT=$W/bench_ipg$DTM/${C}_s${S}
  ...
  (cd $W/i-physgaussian && python -u gs_simulation.py \
     ...
     --config $W/bench_cfg/${C}.json --output_path $OUT.h5dir \
     --output_h5 --implicit --solver newton_gmres --dt_multiplier $DTM \
     --white_bg) >> $W/bench_ipg_${C}_dt$DTM.log 2>&1
  echo "${C} s${S} dt$DTM $((SECONDS - T0)) s" >> $W/bench_ipg_time.log
  PYTHONPATH=$W/anchorflow/lib python -u $W/anchorflow/exe/h5_to_pt.py \
    --dir $OUT.h5dir --out $OUT.pt --cfg $W/bench_cfg/${C}.json \
    >> $W/bench_ipg_${C}_dt$DTM.log 2>&1
done
